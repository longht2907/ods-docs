...
export function ApiReferenceLayout({ children }: { children: ReactNode }) {
  return (
    <div className="grid min-w-0 grid-cols-[minmax(0,1fr)] gap-8 xl:grid-cols-[minmax(0,1fr)_minmax(320px,420px)]">
      {children}
    </div>
  );
}
...
export function ApiReferenceMain({ children }: { children: ReactNode }) {
  return (
    <div className="min-w-0 [&_table]:block [&_table]:max-w-full [&_table]:overflow-x-auto">
      {children}
    </div>
  );
}
